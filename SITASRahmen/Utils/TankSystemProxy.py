...
from SITASRahmen.Components import lamps as l
...
class TankSystemProxy:  
    ...
    def forward(self, obj, state):
       if isinstance(obj, p.Pump):
          if (state):
             self.startPump()
          else:
             self.stopPump()
       elif isinstance(obj, v.Valve):
          if (state):
             self.openValve(obj.getPosition())
          else:
             self.closeValve(obj.getPosition())
       elif isinstance(obj, l.Lamp):
          if (state):
    ...
    def onTimer(self):
        self.components.getCurrentStep().onTimer();

    def onSensor(self, sensorEvent):
        self.components.getCurrentStep().onSensor(sensorEvent)
